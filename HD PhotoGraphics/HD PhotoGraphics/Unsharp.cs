...
namespace HD_PhotoGraphics
{
    public partial class Unsharp : Form
    {
        ...
        {
            double sigma = double.Parse(segma_text_box.Text);
            int N = (int)(3.7 * sigma - 0.5);
            int Mask_Size = 2 * N + 1;
            float[,] GaussMask = new float[Mask_Size, Mask_Size];
            ...
            for (int i = -AddPixel; i <= AddPixel; i++)
            {
                ...
                {
                    GaussMask[i + AddPixel, j + AddPixel] = (float)((1.0 / (2 * Math.PI * Math.Pow(sigma, 2))) * Math.Exp(-(Math.Pow(i, 2) + Math.Pow(j, 2)) / (2 * Math.Pow(sigma, 2))));
                }
            }

            double k = double.Parse(textBox1.Text);
            pictureBox2.Image = display_image((Linear_filter(ref fst_image.image_buffer, ref GaussMask, "no")));
            Bitmap gausimage = new Bitmap(pictureBox2.Image);
            Bitmap normalimage = new Bitmap(pictureBox1.Image);
            Bitmap unsharp = new Bitmap(normalimage.Width , normalimage.Height);
            Color pd = new Color();
            Color pd1 = new Color();
            for (int i = 0; i < normalimage.Height; i++)
            {
                for (int j = 0; j < normalimage.Width; j++)
                {
                    pd1 = gausimage.GetPixel(j, i);
                    pd = normalimage.GetPixel(j, i);
                    float red = (float)(pd.R + (pd.R - pd1.R) * k);
                    float green = (float)(pd.G + (pd.G - pd1.G) * k);
                    float blue = (float)(pd.B + (pd.B - pd1.B) * k);
                    if (red > 255)
                    {
                        red = 255;
                    }
                    ...
                    Color setcolor;
                    setcolor = Color.FromArgb((int)red, (int)green, (int)blue);
                    unsharp.SetPixel(j, i, setcolor);
                }
            }
