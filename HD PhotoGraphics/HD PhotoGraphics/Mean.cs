...
namespace HD_PhotoGraphics
{
    ...
    public partial class Mean : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            dt1 = DateTime.Now;
            int w = int.Parse(Mean_Width.Text);
            int h = int.Parse(Mean_height.Text);
            double value = 1.0 / ((w) * (h));
            float[,] operation_mask = new float[h, w];
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    operation_mask[i, j] = (float)value;
                }
            }
            pictureBox2.Image = display_image((Linear_filter(ref fst_image.image_buffer, ref operation_mask, "no")));
            dt2 = DateTime.Now;
            dt3 = dt2 - dt1;
